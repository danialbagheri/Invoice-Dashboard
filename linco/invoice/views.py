...
def InvoiceUpload(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
                request, 'Unfortunately your form submission was unsuccessful, please try again')
    else:
        form = InvoicesForm()
    ...
    if request.method == 'GET':
        if request.user.is_authenticated():
            if request.GET.get('filter_date'):
                ...
                result = Invoices.objects.filter(
                    invoice_date__month=qmonth, invoice_date__year=qyear).order_by('-invoice_date')
            elif request.GET.get('payment_method'):
                payment_method = request.GET['payment_method']
                result = Invoices.objects.filter(
                    payment_method=payment_method).order_by('-invoice_date')
            elif request.GET.get('organisation'):
                organisation = request.GET['organisation']
                result = Invoices.objects.filter(
                    organisation=organisation).order_by('-invoice_date')
            else:
                context = {
                    'result': page_to_show,
